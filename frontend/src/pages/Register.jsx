  ...
  return (
    <main className="pt-16 min-h-screen flex items-center justify-center px-4 py-8">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 right-1/4 w-64 h-64 rounded-full blur-3xl opacity-15 animate-float" style={{background:'radial-gradient(circle,#DC2626,transparent)'}} />
        <div className="absolute bottom-1/4 left-1/4 w-56 h-56 rounded-full blur-3xl opacity-10 animate-float" style={{background:'radial-gradient(circle,#7F1D1D,transparent)',animationDelay:'2s'}} />
      </div>

      <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} className="w-full max-w-md relative z-10">
        <div className="text-center mb-8">
          <Link to="/" className="inline-flex items-center gap-2 mb-6">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{background:'linear-gradient(135deg,#DC2626,#7F1D1D)'}}>
              <Zap size={20} className="text-white" />
            </div>
            <span className="font-syne font-black text-2xl">RIZER<span className="gradient-text">SPACE</span></span>
          </Link>
          <h1 className="font-syne font-black text-3xl text-white">Create Your Account</h1>
          <p className="text-white/40 text-sm mt-2">Start shopping premium anime figures today</p>
        </div>

        <div className="glass rounded-3xl p-8 border border-white/8">
          <form onSubmit={handle} className="flex flex-col gap-5">
            {[['name','Full Name','text','Your Name'], ['email','Email Address','email','you@example.com']].map(([field,label,type,ph]) => (
              <div key={field}>
          ...
          <p className="text-center text-sm text-white/40">
            ...
            <Link to="/login" className="text-neon-red hover:underline font-semibold">Sign in</Link>
          </p>
        </div>
      </motion.div>
    </main>
