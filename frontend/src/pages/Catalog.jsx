...
const CATEGORIES = ['All', 'Anime Figures', 'Statues', 'Nendoroids', 'Scale Figures', 'Accessories']
const SORT_OPTIONS = [
  { value: '', label: 'Default' },
  ...
  return (
    <main className="pt-24 pb-20 px-4 sm:px-6 lg:px-8 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="mb-10">
          <p className="text-xs text-neon-red font-semibold tracking-widest mb-1">BROWSE ALL</p>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-8">
          ...
          <button onClick={() => setShowFilters(!showFilters)}
            ...
            {hasFilters && <span className="w-2 h-2 rounded-full bg-neon-red" />}
          </button>
          {hasFilters && (
        ...
        {!isLoading && (
          <p className="text-sm text-white/30 mb-6">
            ...
            {category !== 'All' && <span className="text-neon-red"> in {category}</span>}
          </p>
        )}
          ...
          <div className="flex items-center justify-center gap-2 mt-12">
            {Array.from({length: data.pages}).map((_,i) => (
              <button key={i} onClick={() => setPage(i+1)}
                className={`w-10 h-10 rounded-lg text-sm font-bold transition-all ${page === i+1 ? 'btn-primary !p-0' : 'glass border border-white/10 text-white/50 hover:border-neon-red/40'}`}>
                {i+1}
              </button>
            ))}
          </div>
