              ...
              {items.map(item => (
                <motion.div key={item._id} layout initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-20}}
                  ...
                  <div className="flex-1 min-w-0">
                    <Link to={`/product/${item._id}`} className="font-syne font-bold text-sm text-white hover:text-neon-red transition-colors truncate block">
                      {item.title}
                    </Link>
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => dispatch(updateQty({id: item._id, qty: item.qty - 1}))}
                          className="w-7 h-7 rounded-lg glass border border-white/10 flex items-center justify-center hover:border-neon-red/40 transition-all">
                          <Minus size={12} />
                        </button>
          ...
          <div className="lg:col-span-1">
            <div className="glass rounded-2xl p-6 border border-white/8 sticky top-24">
              ...
              <div className="flex flex-col gap-3 mb-6">
                ...
                <div className="flex justify-between text-sm text-white400">
