  ...
  return (
    <main className="pt-16 min-h-screen flex items-center justify-center px-4">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/3 left-1/4 w-64 h-64 rounded-full blur-3xl opacity-15 animate-float" style={{background:'radial-gradient(circle,#DC2626,transparent)'}} />
        <div className="absolute bottom-1/3 right-1/4 w-56 h-56 rounded-full blur-3xl opacity-10 animate-float" style={{background:'radial-gradient(circle,#7F1D1D,transparent)',animationDelay:'3s'}} />
      </div>
      ...
      <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} className="w-full max-w-md relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-white/40 text-sm mt-2">Sign in to your RizerSpace account</p>
        </div>
        ...
        <div className="glass rounded-3xl p-8 border border-white/8">
          <form onSubmit={handle} className="flex flex-col gap-5">
            <div>
              ...
              <input type="email" value={form.email} onChange={e => setForm(f => ({...f, email: e.target.value}))}
                placeholder="you@example.com" required className="input-neon w-full" />
            </div>
            <div>
              <div className="flex justify-between mb-2">
                <label className="text-xs text-white/40 tracking-widest">PASSWORD</label>
                <Link to="/forgot-password" className="text-xs text-neon-red hover:underline">Forgot password?</Link>
              </div>
          ...
          <p className="text-center text-sm text-white/40">
            ...
            <Link to="/register" className="text-neon-red hover:underline font-semibold">Create one</Link>
          </p>
