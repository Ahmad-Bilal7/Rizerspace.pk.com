...
export default function Footer() {
  const cols = [
    ...
    { title: 'Account', links: [['Login', '/login'], ['Register', '/register'], ['My Orders', '/orders'], ['Browse Catalog', '/catalog']] },
    { title: 'Support', links: [['About Us', '/about'], ['FAQ', '/faq'], ['Shipping Policy', '/shipping'], ['Returns', '/returns']] },
  ]
  return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          ...
          <div>
            <Link to="/" className="flex items-center gap-2 mb-4">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{background:'linear-gradient(135deg,#DC2626,#000000)'}}>
            ...
            <div className="flex gap-4 mt-6">
              {[Share2, MessageCircle, Play, Link2].map((Icon, i) => (
                <a key={i} href="#" className="w-9 h-9 rounded-lg glass flex items-center justify-center text-white/40 hover:text-neon-red hover:border-neon-red/40 border border-white/10 transition-all">
                  <Icon size={15} />
                </a>
              ))}
            </div>
            ...
            <div key={col.title}>
              ...
              <ul className="flex flex-col gap-3">
                {col.links.map(([label, to]) => (
                  <li key={label}>
                    <Link to={to} className="text-sm text-white/40 hover:text-neon-red transition-colors">{label}</Link>
                  </li>
                ))}
              </ul>
            </div>
